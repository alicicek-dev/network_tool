...
*   **GitHub Releases ve Sürüm Senkronizasyonu (Version Sync)**: Uygulamanın sürüm kontrol butonu GitHub Releases API'si (`/releases/latest`) üzerinden çalıştığından, `package.json` dosyasındaki sürüm numarası güncellenip koda commitlendiğinde, bu yeni sürümün GitHub üzerinde de bir Release olarak oluşturulması (tag'lenmesi) zorunludur. Aksi takdirde, güncelleme denetleme sistemi en son sürümü eski olarak görerek hatalı çalışacaktır.
    Yeni bir release oluşturmak için GitHub CLI (`gh`) kullanılabilir:
    `$env:GITHUB_TOKEN=$null; gh release create v[Sürüm] --title "NetTool v[Sürüm]" --generate-notes`
